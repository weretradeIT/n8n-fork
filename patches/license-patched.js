...
let License = class License {
    ...
    isLicensed(feature) {
        if (feature === 'feat:showNonProdBanner') return false; // PATCHED: hide non-prod banner
        return true; // PATCHED: all features unlocked
    }
    ...
    getValue(feature) {
        if (feature === 'planName') return 'Enterprise'; // PATCHED
        return -1; // PATCHED: unlimited quota
    }
    getManagementJwt() {
        if (!this.manager) {
            return '';
    ...
    getTeamProjectLimit() {
        return -1; // PATCHED: unlimited projects
    }
    getPlanName() {
